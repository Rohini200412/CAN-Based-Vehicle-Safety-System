...
The system is designed to improve vehicle safety by monitoring engine temperature, controlling vehicle indicators, and detecting reverse obstacles using CAN communication between multiple nodes.
The complete system is divided into three different nodes:
Main Control Node
Indicator Node
Reverse Alert Node
All the nodes communicate using CAN protocol.
The Main Control Node continuously reads engine temperature using the DS18B20 temperature sensor and displays the temperature on the LCD display.
...
In Reverse mode, the Main Node receives obstacle distance data from the Reverse Alert Node through CAN communication.
...
The Indicator Node continuously receives CAN messages from the Main Node and performs left or right scrolling LED operations based on the received data.
Non-blocking scrolling logic is implemented for smooth and responsive indicator control.
